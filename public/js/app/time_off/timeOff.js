...
var dataTable = $('#dataTable').DataTable({
    ...
    Swal.fire({
      ...
      if (result.value) {
        try{
          ...
          if (type === 'POST') {
            ajaxSendForm(apiUrl + '/time_off', 'POST', formAdd);
          } else if (type === 'PUT') {
  ...
  async function blank(){
    $(formAdd).find('select[name=time_off_type_id]').val('');
  ...
  async function renderSelectDestinationType(){
    var req = await ajaxRequest(apiUrl + '/config/time_off_type');
    var data = req.data;
    var optHtml = '<option value="">- Choose Time-off Type -</option>';
  
    for (var i = 0; i < data.length; i++) {
        var row = data[i];
        optHtml += `<option value="${row.id}">${row.title}</option>`;
    }
    $('#timeOffTypeId').html(optHtml);
  }

  async function renderDetail(id){
    try{
      showLoader('#form');
      var req = await ajaxRequest(apiUrl + '/time_off/' + id);
      var data = req.data;
      $(formAdd).find('select[name=time_off_type_id]').val(data.time_off_type_id);
  ...
  async function edit(id) {
    ...
    $(formAdd).attr('action', apiUrl + '/time_off/' + id);
